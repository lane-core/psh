...
Sections psh cares about:

- **§3** — definition of VDCs. Load-bearing for any well-definedness check.
- **§5** — composites and the **Segal condition**. When sequences of horizontal arrows have composites (opcartesian cells). Relevant to pipeline fusibility.
- **§6** — restrictions (vertical arrows acting on horizontal arrows). psh interprets this as interface transformations on channel types (redirections, namespaces).
- **§7** — **virtual equipments**. VDCs with enough restriction structure to serve as a type-theoretic setting. psh's framework is a virtual equipment.
